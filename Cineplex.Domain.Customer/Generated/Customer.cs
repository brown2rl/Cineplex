...
{	
	...
	[DataTable("[dbo].[Customer]", AllowSelect = true, AllowInsert = true, AllowUpdate = true, AllowDelete = true, AllowTruncate = true, EnableCaching = false, CacheTimeout = 0)]
	public partial class Customer : IFlyweight, IRedundancyCheck, INotifyPropertyChanging, INotifyPropertyChanged, IPropertyChangedTrackable
	{
		PropertyStorage Storage = new PropertyStorage(typeof(Customer));
		...
		{
			public static string NameOf<T>(Expression<Func<Customer, T>> property)
			{
				return PropertyOf<Customer>.Name<T>(property);
			}
			
			public static PropertyExpression<Customer> Id { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.Id)); } }
			public static PropertyExpression<Customer> FirstName { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.FirstName)); } }
			public static PropertyExpression<Customer> LastName { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.LastName)); } }
			public static PropertyExpression<Customer> CompanyName { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.CompanyName)); } }
			public static PropertyExpression<Customer> DefaultEmailAddress { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.DefaultEmailAddress)); } }
			public static PropertyExpression<Customer> CustomerSegmentId { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.CustomerSegmentId)); } }
			public static PropertyExpression<Customer> DefaultShipMethodId { get { return new PropertyExpression<Customer>(Properties.NameOf(p => p.DefaultShipMethodId)); } }
		}
	}
}