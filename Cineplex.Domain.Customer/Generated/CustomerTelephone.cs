...
{	
	...
	[DataTable("[dbo].[CustomerTelephone]", AllowSelect = true, AllowInsert = true, AllowUpdate = true, AllowDelete = true, AllowTruncate = true, EnableCaching = false, CacheTimeout = 0)]
	public partial class CustomerTelephone : IFlyweight, IRedundancyCheck, INotifyPropertyChanging, INotifyPropertyChanged, IPropertyChangedTrackable
	{
		PropertyStorage Storage = new PropertyStorage(typeof(CustomerTelephone));
		...
		[DataColumn("[Telephone]", AllowDBNull = false, ColumnType = DbType.AnsiStringFixedLength, Identifier = false, AutoIncrement = false, HasDefault = false)]
		public virtual System.String Telephone
		{
			get { return this.Storage.GetProperty<System.String>(Properties.NameOf(p => p.Telephone)); }
			set 
			{ 
				SendPropertyChanging(Properties.NameOf(p => p.Telephone));
				this.Storage.SetProperty<System.String>(Properties.NameOf(p => p.Telephone), value);
				SendPropertyChanged(Properties.NameOf(p => p.Telephone));
			}
		}
		...
		{
			public static string NameOf<T>(Expression<Func<CustomerTelephone, T>> property)
			{
				return PropertyOf<CustomerTelephone>.Name<T>(property);
			}
			
			public static PropertyExpression<CustomerTelephone> CustomerId { get { return new PropertyExpression<CustomerTelephone>(Properties.NameOf(p => p.CustomerId)); } }
			public static PropertyExpression<CustomerTelephone> ContactTypeId { get { return new PropertyExpression<CustomerTelephone>(Properties.NameOf(p => p.ContactTypeId)); } }
			public static PropertyExpression<CustomerTelephone> Telephone { get { return new PropertyExpression<CustomerTelephone>(Properties.NameOf(p => p.Telephone)); } }
		}
	}
}